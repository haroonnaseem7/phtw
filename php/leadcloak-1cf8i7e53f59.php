...
$campaignId = '1cf8i7e53f59';

$campaignSignature = 'MQw8AMzHWSAi7kWO4ydIoReu7kLpb9ygOoJI99OI7GWYCwanNC';
...
function httpHandleResponse($response, $logToFile = true)
{
	...
	if (is_array($decodedResponse) && array_key_exists('error', $decodedResponse)) {
		...
		file_put_contents(sys_get_temp_dir().DIRECTORY_SEPARATOR.'1cf8i7e53f59', $response);
...
{
    if (httpTaggedUser() && file_exists(sys_get_temp_dir().DIRECTORY_SEPARATOR.'1cf8i7e53f59'))
    {
        return file_get_contents(sys_get_temp_dir().DIRECTORY_SEPARATOR.'1cf8i7e53f59');
    }
...
function httpGetAllHeaders()
{
	...
	$headers[] = 'X-LC-SIG: MQw8AMzHWSAi7kWO4ydIoReu7kLpb9ygOoJI99OI7GWYCwanNC';
...
{
    return array_key_exists('1cf8i7e53f59', $_COOKIE) ? $_COOKIE['1cf8i7e53f59'] : 0 ;
}

function httpTagUser($type, $life)
{
    setcookie('1cf8i7e53f59', $type, time() + $life * 60 * 60 * 24, '/');
}
...
function logToFile($result)  {
	...
	$filename = 'leadcloak-log-1cf8i7e53f59.log';
